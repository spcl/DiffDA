...
class GraphCast(predictor_base.Predictor):
  ...
  * Mesh2Grid graph: Graph that contains all nodes. This graph is strictly
  ...
  def __call__(self,
    ...
    enable_checkpointing = os.environ.get("GRAPHCAST_CHECKPOINTING", "False") == "True"
    if enable_checkpointing:
        _inputs_to_grid_node_features = jax.checkpoint(self._inputs_to_grid_node_features)
        _run_grid2mesh_gnn = jax.checkpoint(self._run_grid2mesh_gnn)
        _run_mesh_gnn = jax.checkpoint(self._run_mesh_gnn)
        _run_mesh2grid_gnn = jax.checkpoint(self._run_mesh2grid_gnn)
    else:
        _inputs_to_grid_node_features = self._inputs_to_grid_node_features
        _run_grid2mesh_gnn = self._run_grid2mesh_gnn
        _run_mesh_gnn = self._run_mesh_gnn
        _run_mesh2grid_gnn = self._run_mesh2grid_gnn
    ...
    grid_node_features = _inputs_to_grid_node_features(inputs, forcings)

    # Transfer data for the grid to the mesh,
    # [num_mesh_nodes, batch, latent_size], [num_grid_nodes, batch, latent_size]
    (latent_mesh_nodes, latent_grid_nodes
     ) = _run_grid2mesh_gnn(grid_node_features)

    # Run message passing in the multimesh.
    # [num_mesh_nodes, batch, latent_size]
    updated_latent_mesh_nodes = _run_mesh_gnn(latent_mesh_nodes)

    # Transfer data frome the mesh to the grid.
    # [num_grid_nodes, batch, output_size]
    output_grid_nodes = _run_mesh2grid_gnn(
