...
def weighted_mse_per_level(predictions: xarray.Dataset,
                           targets: xarray.Dataset,
  ...
  return weighted_error_per_level(predictions,
                                  targets,
                                  per_variable_weights,
                                  mse_loss,
                                  normalized_level_weights)

def weighted_error_per_level(predictions: xarray.Dataset,
                             targets: xarray.Dataset,
                             per_variable_weights: Mapping[str, float],
                             loss_fn,
                             per_level_weights_fn = None
                             ) -> LossAndDiagnostics:
  """
  Compute the loss function weighted per variable and per level.
  Moreover, weights the latitudes to account for unequal distribution of grid points on the sphere.
  """
  def weighted_loss(prediction, target):
    ...
    if 'level' in target.dims:
      loss *= per_level_weights_fn(target).astype(loss.dtype)
    return _mean_preserving_batch(loss)

  losses = xarray_tree.map_structure(weighted_loss, predictions, targets)
...
def single_level_weights(data: xarray.DataArray, level) -> xarray.DataArray:
  """Weights that select a single pressure level."""
  l = data.coords['level']
  weights = xarray.zeros_like(l)
  weights.loc[{'level': level}] = 1  # Replace '1' with the desired non-zero value
  return weights
